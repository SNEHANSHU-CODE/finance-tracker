...
class AnalyticsService {
  ...
  async makeRequest(endpoint, params = {}, options = {}) {
    ...
    for (let i = 0; i <= retries; i++) {
      try {
        ...
        if (useCache) {
          this.setCache(this.getCacheKey(endpoint, params), response.data);
        }
        return response;
      } catch (error) {
        lastError = error;
  ...
  getDateRangeOptions() {
    const now = new Date();
    return {
      'last7days': { label: 'Last 7 Days', startDate: new Date(now.getTime() - 7 * 86400000), endDate: now },
      'last30days': { label: 'Last 30 Days', startDate: new Date(now.getTime() - 30 * 86400000), endDate: now },
      'last90days': { label: 'Last 90 Days', startDate: new Date(now.getTime() - 90 * 86400000), endDate: now },
      'last6months': { label: 'Last 6 Months', startDate: new Date(now.getTime() - 180 * 86400000), endDate: now },
      'last1year': { label: 'Last Year', startDate: new Date(now.getTime() - 365 * 86400000), endDate: now },
      'thisMonth': { label: 'This Month', startDate: new Date(now.getFullYear(), now.getMonth(), 1), endDate: now },
      'lastMonth': { label: 'Last Month', startDate: new Date(now.getFullYear(), now.getMonth() - 1, 1), endDate: new Date(now.getFullYear(), now.getMonth(), 0) },
      'thisYear': { label: 'This Year', startDate: new Date(now.getFullYear(), 0, 1), endDate: now },
      'lastYear': { label: 'Last Year', startDate: new Date(now.getFullYear() - 1, 0, 1), endDate: new Date(now.getFullYear() - 1, 11, 31) }
    };
  }
  ...
  validateDateRange(startDate, endDate) {
    ...
    if (start > now) throw new Error('Start date cannot be in the future');

    const diffDays = Math.ceil(Math.abs(end - start) / 86400000);
    if (diffDays > 730) throw new Error('Date range cannot exceed 2 years');
  ...
  getComparisonPeriods(currentStartDate, currentEndDate) {
    ...
    const previousStart = new Date(start.getTime() - diffDays * 86400000);
  ...
  async measurePerformance(operation, ...args) {
    ...
    try {
      ...
      if (import.meta.env.DEV) {
        console.log(`⚡ Analytics Operation took ${performance.now() - startTime} milliseconds`);
      }
      return result;
    } catch (error) {
      console.error(`❌ Analytics Operation failed after ${performance.now() - startTime} milliseconds:`, error);
      throw error;
    }
  }
